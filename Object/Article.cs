...
namespace WebservicesSage.Object
{
    ...
    public class Article 
    {
        ...
        public Article(IBOArticle3 articleFC)
        {
           ...
           {
               PrixRemiseClient prixC = new PrixRemiseClient();
               prixC.ClinetCtNum = articleTarifClient.Client.CT_Num.ToString();
               prixC.Born_Sup = 1;
               prixC.Price = articleTarifClient.Prix;
               if (string.IsNullOrEmpty(articleTarifClient.Remise.ToString()))
               {
                   prixC.reduction_type = "amount";
               }
               else
               {
                   prixC.reduction_type = "percentage";
                   string[] remise = articleTarifClient.Remise.ToString().Split('%');
                   prixC.RemisePercentage = (double)(Double.Parse(remise[0])) / 100;
               }
               prixC.FixedPrice = articleTarifClient.Prix;
               prixRemisesClient.Add(prixC);
